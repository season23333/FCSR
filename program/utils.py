...
def save_file(corpus, save_path):
    ...
    print('Dataset [{}] was saved at [{}].'.format(corpus.config.dataset_name, save_path))

def load_file(dataset_name, save_path):
    # Load / save dataset
    print('Loading dataset [{}] from [{}].'.format(dataset_name, save_path))
    ret = torch.load(save_path)
    return ret

def get_save_path(dataset_config):
    hasp_tmp = ''
    for k in ['dataset_name', 'embed_dim', 'pretrained', 'nvocab_src', 'nvocab_tgt', 'nvocab', 'entrance']:
        hasp_tmp += '[{} = {}]'.format(k, dataset_config.__dict__[k])
    hasp_tmp = hashlib.md5(hasp_tmp.encode('utf-8')).hexdigest()
    ret = 'datasets/saved/{}_{}.dataset'.format(dataset_config.dataset_name, hasp_tmp)
    return ret

def load_dataset(dataset_config):
    data_path = get_save_path(dataset_config)
    if os.path.exists(data_path) and not dataset_config.force:
        ret = load_file(dataset_config.dataset_name, data_path)
    else:
        print(f'Can\'t find saved dataset [{data_path}]')
        print('Dataset [{}] hasn\'t preprossed yet, Building from raw data.'.format(dataset_config.dataset_name))
        ret = datasets.load_dataset(dataset_config.dataset_name)(dataset_config)
        save_file(ret, data_path)
    ret.loader = {
            'trn': DataLoader(ret.data['trn'], batch_size = dataset_config.batch_size, collate_fn = datasets_utils.pad_sequence, shuffle = True),
            'val': DataLoader(ret.data['val'], batch_size = dataset_config.batch_size, collate_fn = datasets_utils.pad_sequence),
            'tst': DataLoader(ret.data['tst'], batch_size = dataset_config.batch_size, collate_fn = datasets_utils.pad_sequence),
        }
    ret.print_self()
    return ret
...
def manage_extra_output(dict_output):
    ...
    if isinstance(dict_output, dict):
        ...
        for k, v in dict_output.items():
            dict_ret[k] = manage_extra_output(v)
...
def check_cuda(use_gpu):
    if not use_gpu:
        print('GPU(s) cannot be equipped since config.cuda = False.')
        return 0

    num_gpu = torch.cuda.device_count()
    if num_gpu == 0:
        print('GPU(s) are not available, using CPU.')
        return 0

    elif num_gpu == 1 or use_gpu == 1:
        print('Single GPU mode.')
        return 1
    
    elif num_gpu > 1 and use_gpu > 1:
        print('[{}] GPU(s) are found, parallel computing is available.'.format(num_gpu))
        return num_gpu
...
def plot_roc(logit_path):
    ...
    colors = cycle(["#51C1C8", "#E96279", "#44A2D6", "#536D84", "#51C1C8", "#E96279", "#44A2D6", "#536D84", 'aqua', 'darkorange', 'cornflowerblue'])
